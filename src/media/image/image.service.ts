...
export class ImageService {
  ...
  private prefix = process.env.S3_URL_PREFIX;
  ...
  private addPrefixUrl(formats) {
    return Object.entries(formats).reduce((acc, [key, value]) => {
      acc[key] = `${this.prefix}/${value}`;
      return acc;
    }, {});
  }
  ...
  async getImageUrl(id: string) {
    ...
    return {
      raw: `${this.prefix}/${image.raw}`,
      large: this.addPrefixUrl(image.large),
      medium: this.addPrefixUrl(image.medium),
      small: this.addPrefixUrl(image.small),
    };
  }
}
